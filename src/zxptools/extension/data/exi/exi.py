...
from zxptools.extension.data.base import BaseExtensionInfo, ProductElement
from zxptools.extension.data.exi.file_element import (
...
class EXI(BaseExtensionInfo):
    ...
    def __init__(
        ...
        *,
        ...
        files: MutableSequence[EXIFileElement | EXIFileContentsElement]
        | None = None,
        ...
        self.files: MutableSequence[
            EXIFileElement | EXIFileContentsElement
    ...
    def load(
        cls,
        source: StrOrBytesPath | IO[str] | IO[bytes],
    ) -> "EXI":
        ...
        files: list[EXIFileElement | EXIFileContentsElement] = []
        files.extend(
            map(
                lambda file_element: EXIFileElement.load(file_element),
                files_element.iter("file"),
            )
        )
        files.extend(
            map(
                lambda file_contents_element: EXIFileContentsElement.load(
                    file_contents_element
                ),
                files_element.iter("file-contents"),
            )
        )
    ...
    def dump(self) -> etree._Element:
        root_element = etree.Element(
            "extended-extension-info",
            {
                ...
                "type": self.type,
                "requires-restart": (
                    "true" if self.requires_restart else "false"
                ),
        ...
        if not self.files:
            raise Exception("Atleast one FileData in EXI.files is required.")

        files_element = etree.SubElement(root_element, "files")
        for file in self.files:
            element_data = file.dump()
            etree.SubElement(
                files_element, element_data["tag"], element_data["attrib"]
            )
